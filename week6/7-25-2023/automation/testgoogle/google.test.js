...
test("can search Google twice", async () => {
  ...
  const googleSearch = driver.findElement(By.name("q"))
  await googleSearch.sendKeys("cats", Key.RETURN);
  await driver.wait(until.titleIs("cats - Google Search"), 1000);
  ...
  await driver.googleSearch.sendKeys("dogs", Key.RETURN);
  // TODO Wait for the results page to load
  await driver.wait(until.titleIs("dogs - Google Search"), 1000);
});
